...
def sync_imessage(db: Session = Depends(get_db)):
    ...
    synced = 0
    for raw_thread in result["threads"]:
        ext_id = raw_thread["thread_id"]
        ranked = rank_thread(raw_thread)

        contact_handle = raw_thread["handles"][0] if raw_thread["handles"] else "unknown"
        ...
        thread.latest_message = raw_thread.get("latest_message", "")[:500]
        ...
        existing_msg_ids = {m.external_message_id for m in db.query(Message.external_message_id).filter(Message.thread_id == thread.id).all()}
