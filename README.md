...
> which links out to each project: the Surveillance Radar globe at `/radar/`, the
> ReplyGraph demo at `/replygraph/`, and the BetterAsk download page at `/betterask/`.
...
`main`: it runs the Surveillance Radar static export and assembles the portfolio
landing page plus each project page into the published site. For each project's
own setup, see
